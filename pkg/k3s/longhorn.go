...
func InstallLonghorn(host, user, keyPath string) error {
	...
	if _, err := ssh.Run("helm repo add longhorn https://charts.longhorn.io", host, keyPath, user, "", false, 0); err != nil {
		return fmt.Errorf("failed to add Longhorn helm repo: %w", err)
	}

	// Update Helm repositories
	if _, err := ssh.Run("helm repo update", host, keyPath, user, "", false, 0); err != nil {
		return fmt.Errorf("failed to update helm repos: %w", err)
	}

	// Create namespace for Longhorn
	out, err := ssh.Run("kubectl create namespace longhorn-system", host, keyPath, user, "", false, 0)
	if err != nil && !strings.Contains(out, "already exists") {
		return fmt.Errorf("failed to create longhorn namespace: %s: %w", out, err)
	}

	// Install Longhorn using Helm
	if _, err := ssh.Run("helm install longhorn longhorn/longhorn --namespace longhorn-system --set persistence.defaultClass=true", host, keyPath, user, "", false, 0); err != nil {
